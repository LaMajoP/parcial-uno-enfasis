# Evidencias de entrega — Parcial 1

Las capturas incorporadas corresponden a la configuración de producción en AWS.

## Secretos

![AWS Secrets Manager con el secreto de base de datos](evidencias/08-secrets-iam-cloudtrail.jpeg)

Se registra el secreto `emergency-platform/prod/database` sin exponer su valor.

## Configuración dinámica

![AWS AppConfig con el kill switch de auto-despacho habilitado](evidencias/11-appconfig-rollout.jpeg)

AppConfig muestra la versión inicial del kill switch de auto-despacho.

## Presupuesto de AWS

![Presupuesto mensual de AWS de USD 10](evidencias/16-aws-budget.jpeg)

![Alertas de costo real al 50 % y proyectado al 85 %](evidencias/16-aws-budget-1.jpeg)

El presupuesto mensual es de USD 10 e incluye las alertas configuradas al 50 % y al 85 %.

## Video demostrativo

**Enlace:** [Video demostrativo](https://youtu.be/sCtJ_bhDYyQ?si=f32XUleeURkvzIW0)
