"""Notification & Status Broadcast — registra y difunde los cambios de estado."""
import logging
import uuid
...
app = FastAPI(
    title="Emergency Platform — Notification & Status Broadcast",
    version="1.0.0",
    docs_url="/docs",
    lifespan=lifespan,
)
...
def _describe(exc: RequestValidationError) -> str:
    ...
    return f"Invalid notification payload: {location or 'body'}: {message}"
